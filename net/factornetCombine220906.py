...
"""
相对于第一次的Combine来说，参数化的特征学习数量少了，然后补充了BN层；
...
class FactorNetCombine220906small(nn.Module):
    def __init__(self, in_features,
                 out_features=1,
                 time_length=30,
                 dropout = 0.0,
                 ):
        super(FactorNetCombine220906small, self).__init__()
        self.in_ch = in_features
        ...
        self.param_extraction = nn.Sequential(
            nn.Conv1d(in_channels=self.in_ch, out_channels=self.in_ch, kernel_size=self.days, stride=self.days),
            nn.BatchNorm1d(num_features=self.in_ch),
        )
        

        self.ch1 = self.extraction.out_features + self.in_ch # the later is params chs
