...
class FactorNetLSTM_3level(nn.Module):
    def __init__(self, in_features,
                 out_features=1,
                 time_length=30,
                 dropout = 0.0,
                 ):
        super(FactorNetLSTM_3level, self).__init__()
        self.in_ch = in_features
    ...
    def forward(self, x):
        ...
        xfeat10 = self.extraction_k10(x)
        xfeat5 = self.extraction_k5(x)
        xfeat3 = self.extraction_k3(x)
        ...
        xout_10, _ = self.lstm_k10(xfeat10)
        xout_10 = self.bn2_k10(xout_10.transpose(1, 2))
        xout_5, _ = self.lstm_k5(xfeat5)
        xout_5 = self.bn2_k5(xout_5.transpose(1, 2))
        xout_3, _ = self.lstm_k3(xfeat3)
